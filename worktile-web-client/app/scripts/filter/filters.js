...
define(['app'], function (app) {
	...
	app.filter("role", ["$translate",'config',
    function(a,config) {
        return function(b) {
            switch (b) {
            case config.constant.role.admin:
                return a.instant("common.txt_admin");
            case config.constant.role.member:
                return a.instant("common.txt_member");
            case config.constant.role.guest:
                return a.instant("common.txt_visitor");
            default:
                return b
            }
        }
    }]).filter("is_admin", ['config',function(config) {
        return function(a) {
            return a === config.constant.role.admin
        }
    }]).filter("is_member", ['config',function(config) {
        return function(a) {
            return a === config.constant.role.member
        }
    }]).filter("is_guest", ['config',function(config) {
        return function(a) {
            return a === config.constant.role.guest
        }
    }]).filter("is_pending", ['config',function(config) {
        return function(a) {
            return a === config.constant.status.pending
        }
        ...
        return function(a) {
            if (void 0 !== a && null !== a) {
                ...
                return c.isSame(d, "day") ? 
                    b.instant("common.today") + " " + d.format("HH:mm") 
                    : c.add(1, "days").isSame(d, "day") ? 
                        b.instant("common.tomorrow") + " " + d.format("HH:mm") 
                        : c.add( - 1, "day").isSame(d, "day") ? 
                            b.instant("common.yesterday") + " " + d.format("HH:mm") 
                            : c.isSame(d, "year") ? 
    ...
    }]).filter("activityIcon", ["$filter",'config',
    function(a,config) {
        return function(a) {
            var b = a;
            return _.isUndefined(config.activity_icon[a]) || (b = config.activity_icon[a]),
            b
        }
    }]).filter("fullPath", ["$filter",'config',
    function(a,config) {
        return function(a) {
            if (a) return 0 === a.indexOf("/img") ? a: 0 === a.indexOf("http://wt-avatars") ? a: 0 === a.indexOf("http://wt-prj") ? a: config.config.box_url + a
        }
    }]).filter("wtBox", ["$filter",'config',
    function(a,config) {
        return function(a) {
            return a ? config.wtbox_url + a: config.default_box
        }
    }]).filter("wtBoxDownload", ["$filter",'config',
    function(a,config) {
        return function(a) {
            if (_.isEmpty(a)) return config.default_box;
            var b = config.get_cookie("sid"),
            c = a.pid,
            d = a.version ? a.version: "";
            return config.box_url() + "download/" + a.fid + "?pid=" + c + "&token=" + b + "&vid=" + d
        }
    }]).filter("wtExtensionIcon", ["$filter",'config',
    function(a,config) {
        return function(a) {
            return a ? config.cdnpath + "images/extensions/extension_" + a + ".png": config.cdnpath + "images/extensions/extension_normal.png"
        }
    }]).filter("inertPicFromBox", ["$filter",'config',
    function(a,config) {
        return function(a) {
            if (_.isEmpty(a)) return config.default_box;
            if (config.constant.image_exts.indexOf(a.ext) > -1) {
                var b = (config.get_cookie("sid"), a.pid);
                return config.box_url() + "view/" + a.fid + "?pid=" + b
            }
            return _.isUndefined(a.icon) ? void 0 : config.wtbox_url + a.icon
        }
    }]).filter("wtBoxView", ["$filter",'config',
    function(a,config) {
        return function(a) {
            if (_.isEmpty(a)) return config.default_box;
            if (config.constant.image_exts.indexOf(a.ext) > -1) {
                var b = config.get_cookie("sid"),
                c = a.pid;
                return config.box_url() + "view/" + a.fid + "?pid=" + c + "&token=" + b + "&dt=" + (a.dt || "")
            }
            return _.isUndefined(a.icon) ? void 0 : config.wtbox_url + a.icon
        }
    }]).filter("propsFilter",
    ...
    }).filter("getFileIcon", ['config',function(config) {
        return function(a) {
            return _.isEmpty(a) ? "": config.helper.build_file_icon(a)
        }
    }]).filter("fileThumbSrcByExt", [function() {
        return function(a) {
            var a = a.toLowerCase();
            return ["doc", "docx"].indexOf(a) >= 0 ? 
                        "/img/icons/doc.png"
                        : ["ppt", "pptx"].indexOf(a) >= 0 ? 
                            "/img/icons/ppt.png"
                            :["xls", "xlsx"].indexOf(a) >= 0 ? 
                                "/img/icons/xls.png"
                                :"pdf" === a ? 
                                    "/img/icons/pdf.png"
                                    :"txt" === a ? 
        ...
        return function(a, b) {
            ...
            function(a) {
                return a.name.toLowerCase().indexOf(c) > -1 || 
                    a.display_name.toLowerCase().indexOf(c) > -1 || 
                    null != a.email && a.email.toLowerCase().indexOf(c) > -1 || 
                    null != a.phone && a.phone.toLowerCase().indexOf(c) > -1 || 
                    null != a.name_pinyin && a.name_pinyin.toLowerCase().indexOf(c) > -1
            })
        }
    ...
    }]).filter("fileExt", ["$translate",'config',
    function(a,config) {
        return function(b) {
            if (b.type === config.constant.file_type.file) {
                if (config.constant.img_exts.indexOf(b.ext) > -1) return a.instant("common.txt_pic");
                var c = _.findKey(config.constant.exts,
                function(a, c) {
                    return a === b.ext
                });
    ...
    }]).filter("isFile", ['config',function(config) {
        return function(a) {
            return a === config.constant.file_type.file
        }
    }]).filter("isFolder", ['config',function(config) {
        return function(a) {
            return a === config.constant.file_type.folder
        }
    }]).filter("isImageFile", ['config',function(config) {
        return function(a) {
            return ! _.isEmpty(a) && "-1" === config.constant.get_file_icon(a)
        }
    }]).filter("isPreviewFile", ['config',function(config) {
        return function(a) {
            return _.isEmpty(a) ? 
                null
                : a.type && a.type === config.constant.file_type.folder ? 
                    1 
                    : _.contains(config.constant.img_exts, a.ext) ? 
                        0 
                        : _.contains(config.constant.text_exts, a.ext) || _.contains(config.constant.preview_exts, a.ext) ? 
                            2 : null
        }
    ...
    }]).filter("memberStatus", ["$translate",'config',
    function(a,config) {
        return function(b) {
            switch (b) {
            case config.constant.status.pending:
                return a.instant("filter.member_status_pending");
            case config.constant.status.ok:
                return a.instant("filter.member_status_ok")
            }
        }
    }]).filter("projectStatus", ["$translate",'config',
    function(a,config) {
        return function(b) {
            switch (b) {
            case config.constant.archived.yes:
                return a.instant("filter.project_status_archive");
            case config.constant.archived.no:
                return a.instant("filter.project_status_normal")
            }
        }
    }]).filter("fileuploadErrorInfo", ["$translate",'config',
    function(a,config) {
        return function(b) {
            switch (b) {
            case "error":
    ...
    }]).filter("ifImgExtSetClass", ['config',function(config) {
        return function(a) {
            var b = a[0],
            c = a[1];
            return config.constant.image_exts.indexOf(b) > -1 ? c: ""
        }
    }]).filter("getExtIcon", ['config',function(config) {
        return function(a) {
            var b = {
                type: 2,
                ext: config.constant.get_ext(a.substring(a.lastIndexOf(".") + 1))
            };
            return config.config.wtbox_url + config.constant.get_file_icon(b)
        }
    }]).filter("getTemplateBg", ['config',function(config) {
        return function(a) {
            var b = "";
            return b = 1 === a.type ? 
                "url(" + config.cdnpath + "images/dialog_project_create/team-templates/" + a.image + ")"
                : "url(" + config.cdnpath + "images/dialog_project_create/project-templates/" + a.image + ")"
        }
    }]).filter("appClientName", ["$translate",'config',
    function(a,config) {
        return function(b) {
            switch (b.client_id) {
            case config.constant.client.android:
                return a.instant("filter.app_client_name_android");
            case config.constant.client.iphone:
                return a.instant("filter.app_client_name_iphone");
            case config.constant.client.openapp:
                return b.name;
            default:
                return ""
            }
        }
    }]).filter("GetClientEdition", ["$translate",'config',
    function(a,config) {
        return function(b) {
            switch (b.client_id) {
            case config.constant.client.openapp:
    ...
    }]).filter("projectVisibility", ["$translate",'config',
    function(a,config) {
        return function(b) {
            switch (b) {
    ...
    }]).filter("clientName", ["$translate",'config',
    function(a,config) {
        return function(b) {
            switch (b) {
            case config.constant.client.web:
        ...
        return function(a, b, c) {
            return b || angular.isNumber(b) ? 
                (a = a.toString(), b = b.toString(), c ? 
                    ...
                    : a.replace(new RegExp(b, "gi"), '<span class="ui-match">$&</span>')) 
                : a
        }
    })
