...
define(['app'], function (app) {
	 ...
	 *
	 **************************************************************************************************************/
	app.controller('projectHomeCtrl', ["$rootScope", "$scope", "$state", "$stateParams", "$timeout", 
					"globalDataContext", "fastProject", "wtScrollService",'config','Util',
		function (a, b, c, d, e, f, g, h,config,util) {
			...
			var i = (b.vm = {},
				...
				j = function(a) {
					_.findIndex(a, {
						...
						b.project.enable_extension_presenter = !0 
						: 
						b.project.enable_extension_presenter = !1
				},
				k = function(a) {
				...
				l = g;
			l ? (b.project = l, util.project.get_extensions(l.extensions,
					function(a) {
						j(_.filter(a, {
								enable: 1
							})),
							b.project.navigations = _.filter(a, {
								enable: 1,
								type: 1
							}),
							k(l)
					})) : e(function() {
					c.go("project_not_found", {
						pid: i
					}, {
						location: "replace"
					})
			...
			b.sidebar = {
				...
				b.$on(config.constant.event_names.project_extensions_change,
					function(a, b) {
						j(b.extensions)
					})
		 ...
		 *
		 **************************************************************************************************************/
		.controller('projectInfoCtrl', ["$scope", "$rootScope", "$popbox", "globalDataContext", 
						"permissionFilter", "$timeout",'config','Util',
				function (a, b, c, d, e, f, config, util) {
				...
				//     a           b            c              d                    e                f
					...
					h.js_pop_project_menu = function(b) {
						...
						h.js_pop_set_project_logo = function(f) {
					...
					var i = 0,
						j = _.debounce(function(b) {
							...
							50);
					g(),
						a.$watch("project.navigations",
							function(a) {
								j()
							}),
						a.$watch("sidebar.status",
							function(a) {
								j(a)
							}),
						a.$on("$destroy",
